...
public class StudentServiceTests
{
    ...
    public async Task CreateAsync_Should_Create_Brazilian_Student_Successfully()
    {
        ...
        var result = await new StudentService(context).CreateAsync(
            CreateDto(1, nationality: "Brasil", cpf: "12345678901", passport: null));
    ...
    public async Task CreateAsync_Should_Create_International_Student_Successfully()
    {
        ...
        var result = await new StudentService(context).CreateAsync(
            CreateDto(1, nationality: "Argentina", cpf: "99999999999", passport: "PASSPORT-123"));
    ...
    public async Task CreateAsync_Should_Reject_When_Brazilian_Without_Cpf()
    {
        ...
        var result = await new StudentService(context).CreateAsync(
            CreateDto(1, nationality: "Brasil", cpf: null, passport: "PASS-123"));

        result.Errors.Should().ContainSingle()
            .Which.Should().Be("Documento de identificação inválido para a nacionalidade informada.");
    }
    ...
    public async Task CreateAsync_Should_Reject_Duplicate_Registration_Number()
    {
        ...
        {
            UserId = 1, User = firstUser, Cpf = "11111111111", RegistrationNumber = "REG-1",
            BirthDate = new DateTime(2000, 1, 1), Phone = "12345678", Address = "Test Address", Nationality = "Brasil"
        });
        await context.SaveChangesAsync();

        var result = await new StudentService(context).CreateAsync(
            CreateDto(2, nationality: "Brasil", cpf: "22222222222", passport: null, regNumber: "REG-1"));

        result.Errors.Should().ContainSingle()
            .Which.Should().Be("Já existe um aluno cadastrado com esta Matrícula, CPF ou Passaporte.");
    }
    ...
    public async Task DeleteAsync_Should_Prevent_Deletion_If_Enrollments_Exist()
    {
        ...
        {
            UserId = 1, User = user, Cpf = "11111111111", RegistrationNumber = "REG-1", Nationality = "Brasil"
        };
        context.Users.Add(user);
        context.Students.Add(student);
        context.Enrollments.Add(new Enrollment { Id = 1, StudentId = 1, CourseId = 1, FormatId = 1, StatusId = 1 });
        await context.SaveChangesAsync();

        var result = await new StudentService(context).DeleteAsync(1);

        result.Errors.Should().ContainSingle()
            .Which.Should().Be("Não é possível remover o aluno, pois existem matrículas ativas vinculadas a ele.");
    ...
    private static StudentCreateDto CreateDto(int userId, string nationality, string? cpf, string? passport, string regNumber = "REG-NEW") => new()
    {
        UserId = userId,
